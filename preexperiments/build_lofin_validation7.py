"""Build validation7: self-constructed same-company multi-filing SEC set.

The LOFin test subsets contain no same-company multi-filing questions whose
company is fresh (all 48 same-year rows overlap the 114 tickers used by the
first six validation batches).  This builder therefore constructs questions
from SEC 10-Q/10-K filings of six fresh companies, with gold obligations
derived from (a) which filing physically reports the requested period figure
(XBRL companyfacts, accession-matched to the downloaded filing) and (b) a
text-locality check that the value appears near a revenue label in the corpus
text.  The manifest (question templates, case selection, dev/frozen split) is
frozen before this builder runs; no method results exist yet.
"""
...
def locate_value(value: int, text: str) -> Dict[str, object] | None:
    ...
    for form in candidates:
        ...
        for match in re.finditer(re.escape(form), normalized):
            start = match.start()
            window = normalized[max(0, start - 250): start + 60]
...
def pick_concept(facts: Mapping[str, object], required: Sequence[tuple[str, str, str]]) -> str | None:
    ...
    for concept in REVENUE_CONCEPTS:
        ...
        if all((start, end) in keyed for start, end in required):
            return concept
    return None
...
def build(
    ...
    if overlap:
        raise ValueError(f"validation7 overlaps prior documents: {overlap}")
    if ticker_overlap:
        raise ValueError(f"validation7 overlaps prior companies: {ticker_overlap}")
    ...
    for ticker in tickers:
        ...
        for fact_key, start, end, form in required:
            ...
            matches = [
                item
                for item in all_rows.get((start, end), [])
    ...
    result = {
        "schema": "finplan-lofin-validation7.v1",
...
def main() -> None:
    ...
    parser.add_argument("--manifest", default="preexperiments/lofin_validation7_manifest.json")
    parser.add_argument("--out", default=DATA_ROOT / "lofin_validation7_v1.json")
    parser.add_argument(
        "--prior",
        nargs="+",
        default=[
