"""FinPlan v8 (runner file v6): v7 + half-year alias mapping.

Adds the half-year aliases pre-declared in the validation7 manifest
(interpretation_boundary: H1 -> Q2 10-Q, H2 -> Q3 10-Q + FY 10-K).  All other
logic is identical to run_nonoracle_obligation_planning_v5.py.
"""
...
def run(data_path: Path, registry_path: Path, out_path: Path, budget: int) -> Mapping[str, object]:
    ...
    result = {
        ...
        "status": "post-validation7-frozen-not-confirmatory",
        "protocol": {
            "data_sha256": hashlib.sha256(data_path.read_bytes()).hexdigest(),
            "registry_sha256": hashlib.sha256(registry_path.read_bytes()).hexdigest(),
            "source_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            "cases": len(data["cases"]),
            "planner": "entity + filing type + fiscal year + fiscal period obligations + half-year aliases (H1->Q2, H2->Q3+FY)",
            "gold_fields_hidden_from_planner": ["candidate_legs", "candidate_obligations", "gold_doc_ids", "gold_pages", "reference_answer"],
            "budget": budget,
        },
        "interpretation_boundary": [
            "v8 = v7 + half-year alias mapping (H1->Q2 10-Q, H2->Q3 10-Q + FY 10-K), pre-declared in the validation7 manifest before any method ran.",
            "The alias questions on validation7 confirm or refute this pre-declared mapping; they are not a tuning signal.",
...
def main() -> None:
    ...
    parser.add_argument("--out", default=RESULTS_ROOT / "nonoracle_obligation_v8_validation7_frozen.json")
