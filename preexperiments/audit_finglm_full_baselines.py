"""FinGLM full-run audit (Task #21): pairwise significance tests per protocol.

Input: results/finglm_full_baselines.json (rows: case_id/stratum/method/
leg_recall/closure/wrong_doc_rate/queries/documents).

Tests, identical procedures to the LOFin extended tests
(audit_lofin_full_extended_tests.py):
- closure: exact McNemar on the paired {0,1} closure indicators;
- wrong_doc_rate: two-sided paired permutation test on the mean diff
  (sign-flip, seed 20260812, 20,000 permutations, +1 pseudo-count);
- applied on the overall layer and on each of the 5 answer_type strata.

No multiple-comparison adjustment is applied; all p-values are reported
in full (same disclosure as the LOFin audit).  Differences are reported
honestly, including non-significant ones.

Usage: python audit_finglm_full_baselines.py [bench_path] [out_path]
"""
...
OUT = RESULTS / "finglm_full_audit_v1.json"

METHODS = ["single_shot", "period_metadata_decomposition",
           "generic_adaptive_period", "hirec_period"]
PERMUTATIONS = 20_000
SEED = 20260812  # identical to the LOFin extended tests
...
def main() -> None:
    ...
    # per-method summary on the overall layer, mirrored from the bench summary
    summary = bench.get("summary", {})
    out = {
        "schema": "finplan-finglm-full-audit.v1",
        "protocol": "docs/04-数据与实验/finglm_protocol_v1.json",
