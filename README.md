...
- 当前方法状态：把义务扩展为“主体、filing type、财政年度、财政期间”四元键的 v7 在 validation5 错误驱动回放达到 8/8；新冻结 validation6 只达 6/8，未超过同元数据强基线。validation7 自建冻结集上，v8（v7 + 预声明半年度别名映射 H1→Q2、H2→Q3+FY）闭包 11/14，首次在同一冻结集上显著超过元数据分解与自身 v7（p≈.03），但相对 Generic/HiREC-style 仅方向有利（p=0.25）；同一冻结集 A4 消融（预声明一次性）显示 v8 在移除 available_at 后 0 未来泄漏、case 级不变（防泄漏结构性），Single-shot/Generic/HiREC 显著泄漏（p≤.016）。
- **validation8（v9 修改确认集，2026-08-11）**：6 家全新公司（CSX/CVS/GD/TXN/CMCSA/HCA）× 5 文件 = 30 份此前未用 SEC 文件，30 题（dev 6 / frozen 24），54 条 XBRL+文本双验证，manifest 在方法运行前冻结 v9 两处解析修改（"fiscal year N"→(N,10-K,FY)；"first n quarters of Y"→只取 Qn 累积文件）。frozen 24 一次性：v9 闭包 24/24、wrong_doc 0.000，显著高于四个 v7-shared baselines（10–11/24，配对精确 McNemar **p≤0.00024**，全部非平局配对单侧）；fy_q1n 5/5（v8 0/5，P1 确认）、h1_9m 错误文档 0.000 vs v7/v8 0.333（P2 确认）。**诚实负结果（P4，预声明）**：与 v9 共享义务、仅检索退化为元数据分解 fill_missing 的对照 24/24 完全持平——策略优势主张收缩到表示层（期间解析），级联检索策略无独立于表示的增益；v9 vs v8 总体 p=0.0625（方向性）。future_leak 0/24，dev↔frozen 确定性 0 差异。A3 在本轨道无 L 机制可删（SEC pilot 已有 finplan_no_limit 证据），A5/A6 载体数据未构建；真实 reader/LLM、页级证据、原版 HiREC/Search-R1、停止/拒答仍未完成。
